...
- Data: a GitHub Actions cron job runs every 30 minutes with an EVE SSO refresh
  ...
  against Jita min-sell from Fuzzwork, and commits `data.json`.
...
- **Settings → Secrets and variables → Actions**: add `EVE_CLIENT_ID`,
  `EVE_CLIENT_SECRET`, `EVE_REFRESH_TOKEN`.
...
```bash
EVE_CLIENT_ID=... EVE_CLIENT_SECRET=... EVE_REFRESH_TOKEN=... node scripts/fetch-data.mjs
```
